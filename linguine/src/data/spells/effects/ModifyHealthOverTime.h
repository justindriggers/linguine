...
class ModifyHealthOverTime : public Effect {
  public:
    ModifyHealthOverTime(EntityManager& entityManager, Type type,
                         glm::vec3 color, float duration, uint32_t ticks,
                         int32_t powerPerTick)
        : Effect(type, color, duration, ticks), _entityManager(entityManager),
